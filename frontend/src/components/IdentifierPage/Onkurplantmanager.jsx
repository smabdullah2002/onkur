    ...
    return (
        ...
        >
            <div className="relative h-40 overflow-hidden">
                ...
                <div
                    className={`absolute right-3 top-3 flex items-center gap-1 rounded-full px-2 py-1 text-xs font-semibold backdrop-blur-sm ${plant.directSunlight ? "bg-amber-400/85 text-amber-900" : "bg-slate-400/80 text-slate-800"
                        }`}
                >
...
function AddPlantModal({ onClose, onAdd, onIdentify }) {
    ...
    const hasCustomWaterFreq =
        waterFreq !== "" &&
        !WATER_FREQ_OPTIONS.some((option) => Number(option.value) === Number(waterFreq));
            ...
            if (identified?.plant_name) {
                ...
                const confidenceText =
                    typeof identified.confidence === "number"
                        ...
                        : "";
                setIdentifyMessage(
                    `AI suggestion: ${identified.plant_name}${confidenceText}${identified.bangla_name ? ` | Bangla: ${identified.bangla_name}` : ""
                    }`
                );
            } else {
                ...
                <div className="space-y-5 p-6">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <select
                                value={waterFreq}
                                onChange={(event) => setWaterFreq(Number(event.target.value))}
                            ...
                            >
                                {hasCustomWaterFreq && (
                                    <option value={waterFreq}>{`Every ${waterFreq} days (AI)`}</option>
                                )}
                        ...
                        <div className="flex overflow-hidden rounded-xl border-[1.5px] border-[#2a3d2a]">
                            {[
                                ...
                                <button
                                    ...
                                    className={`flex flex-1 items-center justify-center gap-2 py-2.5 text-sm font-medium transition-all duration-200 ${directSunlight === option.value
                    ...
                    <header className="sticky top-0 z-40 grid grid-cols-[1fr_auto_1fr] items-center border-b border-[#2a3d2a] bg-[#0f1a10]/85 px-6 py-4 backdrop-blur-xl">
                        ...
                        <div className="flex gap-2 rounded-xl border border-[#2a3d2a] bg-[#1a2a1a] p-1">
                            ...
                            <button
                                ...
                                className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors ${activePage === 'routine' ? 'bg-[#5c9e4a] text-white' : 'text-[#8ab87a]'
                                    }`}
                            >
                                Routine
                            </button>
                            ...
                            <div className="flex gap-0.5 overflow-hidden rounded-xl border border-[#2a3d2a] bg-[#1e2e1e] p-0.5">
                                {["grid", "list"].map((currentView) => (
                                    <button
                                        ...
                                        className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition-all duration-200 ${view === currentView ? "bg-[#5c9e4a] text-white" : "text-[#7da56a]"
                                            }`}
                                    >
                                        {currentView === "grid" ? (
